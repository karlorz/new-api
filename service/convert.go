...
func StreamResponseOpenAI2Claude(openAIResponse *dto.ChatCompletionsStreamResponse, info *relaycommon.RelayInfo) []*dto.ClaudeResponse {
	...
	finish := func(usage *dto.Usage, defaultStopReason string) {
		stopOpenBlocks()
	...
	emitThinking := func(reasoning string) {
		if reasoning == "" {
			return
		}
	...
	if len(openAIResponse.Choices) == 0 {
		if openAIResponse.Usage != nil || info.FinishReason != "" {
			usage := openAIResponse.Usage
			if usage == nil {
				usage = info.ClaudeConvertInfo.Usage
			}
			finish(usage, "end_turn")
		}
		return claudeResponses
	}

	chosenChoice := openAIResponse.Choices[0]
	emitThinking(chosenChoice.Delta.GetReasoningContent())
	emitText(chosenChoice.Delta.GetContentString())
	emitTools(chosenChoice.Delta.ToolCalls)
